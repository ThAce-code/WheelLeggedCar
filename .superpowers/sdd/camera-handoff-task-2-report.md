...
### Final P2 attach classification

The final review corrected camera-control magic classification without changing the ABI: `magic == 0` is the only unpublished startup state and retries silently; a nonzero value different from `INTERCORE_CAMERA_MAGIC` is a published invalid layout and increments `invalid_layout_count`; correct magic with a producer epoch that has not caught up remains a silent not-ready retry. A host test observed RED on the nonzero bad-magic counter, then all three classification cases passed after the minimal change. Camera host/static, foundation host, and diff checks passed. Incremental IAR builds passed for CM7_0 (0 errors, the same 3 pre-existing `Pe550` warnings) and CM7_1 (0 errors/0 warnings). Hardware was not rerun, so Gate 1 remains `FAIL`, Gate 2 remains `NOT RUN`, and status remains `DONE_WITH_CONCERNS`.
