...
The approved handoff design and implementation plan now match camera-control version 2: mirror fields consume the former reserve, `consumer_last_sequence`/age remain at offsets 172/176, `producer_period_drop_count` is at offset 184, and the trailing reserve starts at offset 188 with 68 bytes. Observation publication is permitted only after successful release with data fields → DMB → sequence → DMB. Capture static, camera static, both host tests, and `git diff --check` are the required software regression gate for this correction.
