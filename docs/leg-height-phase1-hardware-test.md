...
Current status: offline PowerShell checks are runnable in this workspace; IAR build has not been rerun after the legacy stance-map update.
...
| Gate | Build SHA | Legacy stance start/end (units) | Safe-pose BODY_WHEEL measurement | Max pitch (deg) | Max wheel RPM | IK margin min | IK faults | Safety trips | Result | Notes |
|---|---|---:|---:|---:|---:|---:|---:|---:|---|---|
| bench/no wheel output | working tree | TBD | TBD | TBD | TBD | TBD | TBD | TBD | needs rerun | Rebuild after the legacy stance-map update; confirm servo PWM behavior with wheel/motor output disabled. |
| supported stationary at low/default/high legacy stance | working tree | 30 / 55 / 80 | TBD | TBD | TBD | TBD | TBD | TBD | needs rerun | Validate the empirical stance range one endpoint at a time; these are legacy stance units, not measured physical height. |
| LH S-curve re-target (supported) | TBD | 55 -> 65 -> 45 -> 55 | TBD | TBD | TBD | TBD | TBD | TBD | not run | Run `collect_balance_data.ps1 -Port COM6 -Duration 18 -Commands "0:STOP;2:LH,55;4:LH,65;6:LH,45;12:LH,55" -Out data\phase1_gate1_lh_scurve_retarget.csv -Note phase1_gate1_lh_scurve_retarget`; record legacy-stance target-versus-output mismatch, fault state, and observed jerk. A 55-unit stationary shake is a hardware/servo-hold investigation. |
...
| low-speed straight transition | TBD | 45 -> 65 -> 45 | TBD | TBD | TBD | TBD | TBD | TBD | not run | Confirm 30 RPM transition cap and fast-drive interlock during legacy stance motion. |
...
`LHF,<legacy_stance_units>` is the fast legacy-stance profile for response testing. It
uses the same 30–80 legacy stance units empirical map as `LH`, but completes the reference
...
calibration, and all other paths retain the 90 °/s cap. This permits a 30→80-unit
reference move to be evaluated without making normal low-speed driving
more aggressive.
...
Pass only if `leg_fault_reason` remains zero, `leg_legacy_stance_ref_units` stays within
30–80 legacy stance units, each move reaches its target without reversing past it,
and no linkage interference, supply sag, or wheel movement occurs. A failed
bench result blocks any moving-chassis test.

After the two 25-unit endpoint moves pass, run the 50-unit fast-span comparison
...
5. Use `LXY` only for absolute BODY_WHEEL points selected at least 2 mm inside
   the calibrated hull; verify `LXY,0,55` rejects without motion.
...
`LJ,<legacy_stance_units>` is excluded from normal Phase 1 gates. To build the temporary
...
empirical `LH/LHF` legacy stance map. Complete the reference-pose and
motor-disabled physical-coordinate procedure before considering dynamic 2D IK
or wheel-shift work.
