...
def calibrate_fisheye_from_frames(
    ...
    """Run fisheye calibration using the same chessboard observations."""
    if len(objpoints_all) < 3:
        return None, None, float("nan"), []
    ...
    for object_points_one, image_points_one, rvec, tvec in zip(
        ...
        per_view.append(float(cv2.norm(observed_xy, projected_xy, cv2.NORM_L2)
                              / len(projected_xy)))
    return matrix, dist, float(rmse), per_view


def select_model(standard_rms: float, fisheye_rms: float) -> str:
    """Use fisheye only when it improves RMS by at least five percent."""
    if not np.isfinite(fisheye_rms):
        return "standard"
    if not np.isfinite(standard_rms):
        return "fisheye"
    return "fisheye" if fisheye_rms <= standard_rms * 0.95 else "standard"


def _canonical_calibration_bytes(result: dict) -> bytes:
    """Return the stable payload used by JSON and the firmware CRC."""
    payload = {key: value for key, value in result.items()
               if key != "calibration_crc32"}
    return json.dumps(payload, sort_keys=True, separators=(",", ":"),
                      ensure_ascii=False).encode("utf-8")


def _calibration_crc32(result: dict) -> int:
    return zlib.crc32(_canonical_calibration_bytes(result)) & 0xFFFFFFFF


def _to_q(value: float, fractional_bits: int) -> int:
    scaled = int(round(float(value) * float(1 << fractional_bits)))
    if not -(1 << 31) <= scaled < (1 << 31):
        raise ValueError(f"fixed-point value out of int32 range: {value}")
    return scaled


def export_c_header(result: dict, output_path: Path) -> None:
    """Export a deterministic fixed-point calibration header for firmware."""
    width, height = result["image_size"]
    if width <= 0 or height <= 0:
        raise ValueError("image_size must be positive")
    matrix = np.asarray(result["K"], dtype=np.float64).reshape(3, 3)
    distortion = np.asarray(result["D"], dtype=np.float64).reshape(-1)
    if result["model"] not in ("standard", "fisheye"):
        raise ValueError("model must be standard or fisheye")

    crc32 = int(result.get("calibration_crc32", _calibration_crc32(result)))
    distortion_q29 = [_to_q(value, 29) for value in distortion]
    distortion_q29.extend([0] * (8 - len(distortion_q29)))
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    guard = "_MT9V03X_CALIBRATION_H_"
    lines = [
        "/* Generated by tools/calibration/camera_calibrate.py. */",
        f"#ifndef {guard}",
        f"#define {guard}",
        "",
        f"#define CAMERA_CALIBRATION_WIDTH ({int(width)}U)",
        f"#define CAMERA_CALIBRATION_HEIGHT ({int(height)}U)",
        "#define CAMERA_CALIBRATION_MODEL_STANDARD (0U)",
        "#define CAMERA_CALIBRATION_MODEL_FISHEYE (1U)",
        "#define CAMERA_CALIBRATION_MODEL (CAMERA_CALIBRATION_MODEL_" +
        ("FISHEYE" if result["model"] == "fisheye" else "STANDARD") + ")",
        f"#define camera_calibration_crc32 (0x{crc32:08X}UL)",
        f"#define CAMERA_CALIBRATION_FX_Q16 ({_to_q(matrix[0, 0], 16)}L)",
        f"#define CAMERA_CALIBRATION_FY_Q16 ({_to_q(matrix[1, 1], 16)}L)",
        f"#define CAMERA_CALIBRATION_CX_Q16 ({_to_q(matrix[0, 2], 16)}L)",
        f"#define CAMERA_CALIBRATION_CY_Q16 ({_to_q(matrix[1, 2], 16)}L)",
    ]
    for index, value in enumerate(distortion_q29):
        lines.append(f"#define CAMERA_CALIBRATION_D{index}_Q29 ({value}L)")
...
def _build_calibration_result(
    ...
    result = {
        ...
        "K": np.asarray(matrix, dtype=np.float64).reshape(3, 3).tolist(),
        "D": np.asarray(distortion, dtype=np.float64).reshape(-1).tolist(),
        "rms": float(rms),
        "per_view_rms": [float(value) for value in per_view],
        "board": {"cols": int(cols), "rows": int(rows),
                  "square_mm": float(square_mm)},
    }
    result["calibration_crc32"] = _calibration_crc32(result)
    return result


def _save_batch_calibration(result: dict, output_npz: Path,
                            output_json: Optional[Path]) -> None:
    """Save batch results without changing the legacy interactive format."""
    output_npz = Path(output_npz)
    output_npz.parent.mkdir(parents=True, exist_ok=True)
    matrix = np.asarray(result["K"], dtype=np.float64)
    distortion = np.asarray(result["D"], dtype=np.float64)
    np.savez(
        output_npz,
        camera_index=-1,
        backend="batch",
        resolution=np.asarray(result["image_size"], dtype=np.int32),
        camera_matrix=matrix,
        dist_coeffs=distortion,
        rmse=result["rms"],
        per_view_errors=np.asarray(result["per_view_rms"], dtype=np.float32),
        cols=result["board"]["cols"],
        rows=result["board"]["rows"],
        square_mm=result["board"]["square_mm"],
        schema_version=result["schema_version"],
        model=result["model"],
        calibration_crc32=result["calibration_crc32"],
    )
    json_path = Path(output_json) if output_json is not None else output_npz.with_suffix(".json")
...
def calibrate_from_images(
    ...
    *,
    ...
    detect_fn: Optional[Callable[[np.ndarray, int, int], Optional[np.ndarray]]] = None,
    output_json: Optional[Path] = None,
    output_header: Optional[Path] = None,
) -> dict:
    """Batch calibrate native-resolution images and return a portable result.

    ``expected_size`` rejects a mode mismatch before corner extraction.  The
    optional ``detect_fn`` exists for deterministic tests; production callers
    use the normal chessboard detector.
    """
    if model not in ("auto", "standard", "fisheye"):
        raise ValueError("model must be auto, standard, or fisheye")
    detector = detect_fn if detect_fn is not None else detect_chessboard
    objp = np.zeros((cols * rows, 3), np.float32)
    objp[:, :2] = np.mgrid[0:cols, 0:rows].T.reshape(-1, 2)
    objp *= square_mm
    ...
    for path in image_paths:
        ...
        if expected_size is not None and current_size != expected_size:
            raise ValueError(
                f"image resolution {current_size[0]}x{current_size[1]} does not match "
                f"expected {expected_size[0]}x{expected_size[1]}")

        corners = detector(img, cols, rows)
        if corners is not None:
            objpoints.append(objp)
            imgpoints.append(corners)
    ...
    if not objpoints or image_size is None:
        raise ValueError("no valid chessboard frames")

    standard_matrix, standard_distortion, standard_rms, standard_per_view = calibrate_from_frames(
        objpoints, imgpoints, image_size)
    if standard_matrix is None or standard_distortion is None:
        raise ValueError("standard calibration requires at least three valid frames")

    selected_model = "standard"
    matrix = standard_matrix
    distortion = standard_distortion
    rms = standard_rms
    per_view = standard_per_view
    if model in ("auto", "fisheye"):
        fisheye_matrix, fisheye_distortion, fisheye_rms, fisheye_per_view = (
            calibrate_fisheye_from_frames(objpoints, imgpoints, image_size))
        if model == "fisheye":
            if fisheye_matrix is None or fisheye_distortion is None:
                raise ValueError("fisheye calibration failed for the supplied frames")
            selected_model = "fisheye"
            matrix = fisheye_matrix
            distortion = fisheye_distortion
            rms = fisheye_rms
            per_view = fisheye_per_view
        elif (fisheye_matrix is not None and fisheye_distortion is not None
              and select_model(standard_rms, fisheye_rms) == "fisheye"):
            selected_model = "fisheye"
            matrix = fisheye_matrix
            distortion = fisheye_distortion
            rms = fisheye_rms
    ...
    result = _build_calibration_result(
        selected_model, image_size, matrix, distortion, rms, per_view,
        cols, rows, square_mm)
    if output_path is not None:
        _save_batch_calibration(result, output_path, output_json)
    ...
    if output_header is not None:
        export_c_header(result, output_header)
...
def main() -> int:
    ...
    parser.add_argument("--output-npz", type=Path, default=None,
                        help="Batch calibration NPZ output (overrides --output)")
    parser.add_argument("--output-json", type=Path, default=None,
                        help="Batch calibration JSON output")
    parser.add_argument("--output-header", type=Path, default=None,
                        help="Optional generated firmware calibration header")
    parser.add_argument("--images", type=str, default=None,
                        help="Glob pattern for batch calibration from image files")
    parser.add_argument("--model", choices=["auto", "standard", "fisheye"],
                        default="auto", help="Calibration lens model")
    parser.add_argument("--expected-width", type=int, default=None,
                        help="Reject batch images whose width differs")
    parser.add_argument("--expected-height", type=int, default=None,
                        help="Reject batch images whose height differs")
    ...
    if args.images:
        ...
        expected_size = (args.expected_width, args.expected_height) \
            if args.expected_width is not None else None
        try:
            calibrate_from_images(
                paths, args.cols, args.rows, square_mm,
                args.output_npz if args.output_npz is not None else args.output,
                model=args.model,
