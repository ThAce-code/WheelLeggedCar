...
Assert-Contains "project/code/app_types.h" "forward_ramped_rpm" "chassis_output_struct must expose the ramped forward target."
...
Assert-Contains "project/code/control_balance.c" "drive_forward_target_rpm = chassis->forward_ramped_rpm" "Balance diagnostics must retain the ramped forward target."
Assert-Contains "project/code/control_balance.c" "drive_forward_actual_rpm = chassis->forward_actual_rpm" "Balance diagnostics must retain measured forward speed."
...
Assert-Contains "project/code/telemetry.c" 'float vofa_data\[72\]' "Telemetry must emit the 72-channel race diagnostics frame."
...
Assert-Contains "project/code/telemetry.c" 'vofa_data\[62\]\s*=\s*chassis->forward_target_rpm' "Telemetry index 62 must publish the bounded forward target."
Assert-Contains "project/code/telemetry.c" 'vofa_data\[63\]\s*=\s*chassis->forward_ramped_rpm' "Telemetry index 63 must publish the ramped forward target."
Assert-Contains "project/code/telemetry.c" 'vofa_data\[64\]\s*=\s*chassis->wheel_speed_measured_rpm' "Telemetry index 64 must publish measured wheel speed."
Assert-Contains "tools/collect_balance_data.ps1" '\$FloatCount = 72' "Collector must parse 72-channel telemetry."
...
Assert-Contains "tools/collect_balance_data.ps1" "race_assist_enable" "Collector must write race enable."
Assert-Contains "tools/collect_balance_data.ps1" "race_u_actual" "Collector must write race actual request."
Assert-Contains "tools/collect_balance_data.ps1" "balance_output_limit_rpm" "Collector must write active balance cap."
