...
Assert-Match $cameraConsumer '(?s)if\s*\(\s*\(uint8\)CAMERA_CONSUMER_LINK_CONNECTED\s*!=\s*consumer_diag\.socket_state\s*\)\s*\{\s*camera_frame_consumer_try_network\s*\(\s*\)\s*;\s*\}\s*.*?intercore_camera_consumer_acquire_latest' 'CM7_1 returns before acquiring a frame when WiFi is disconnected'
...
Assert-Match $cameraConsumerHeader 'typedef\s+void\s*\(\*camera_frame_handler_fn\)\(const\s+camera_vision_frame_view_struct\s*\*frame\)' 'camera consumer frame handler contract is missing'
Assert-Match $cameraConsumer '(?s)camera_frame_handler\s*\(\s*&vision_view\s*\).*?seekfree_assistant_camera_send' 'vision handler does not run before the optional WiFi send'
...
Assert-Match $cm7Main '(?s)while\s*\(\s*true\s*\)\s*\{.*?single_gap_app_service\s*\(\s*camera_frame_consumer_now_ms\s*\(\s*\)\s*\)\s*;.*?camera_frame_consumer_service\s*\(\s*\)\s*;' 'CM7_1 main loop does not service single-gap sensing before the frame consumer'
