...
$numericFieldHelper = Get-CFunction $source 'gnss_copy_numeric_field'
$transactionHelper = Get-CFunction $source 'gnss_parse_sentence_transaction'
$hostParser = Get-CFunction $source 'gnss_host_parse_sentence'
...
Assert-Match $ggaParser 'gnss_get_int_number\(&buf\[get_parameter_index\(6,\s*buf\)\],\s*1U,\s*&fix_quality\)' 'GGA quality is not validated with bounded field extraction'
Assert-Match $ggaParser 'gnss->fix_quality\s*=\s*\(uint8\)fix_quality\s*;' 'GGA field 6 Fix Quality is not assigned by gps_gngga_parse'
Assert-Match $ggaParser 'gnss_get_int_number\(&buf\[get_parameter_index\(7,\s*buf\)\],\s*1U,\s*&satellite_used\)' 'GGA satellite count is not validated with bounded field extraction'
Assert-Match $ggaParser 'gnss_get_float_number\(&buf\[get_parameter_index\(8,\s*buf\)\],\s*1U,\s*&hdop\)' 'GGA HDOP is not validated with bounded field extraction'
Assert-Match $ggaParser 'gnss->satellite_used\s*=\s*\(uint8\)satellite_used\s*;' 'GGA field 7 satellite count is not assigned by gps_gngga_parse'
Assert-Match $ggaParser 'gnss->hdop\s*=\s*hdop\s*;' 'GGA field 8 HDOP is not assigned by gps_gngga_parse'
...
Assert-Match $numericFieldHelper 'index\s*\+\s*1U\s*>=\s*capacity' 'numeric field extraction is not capacity bounded'
Assert-Match $numericFieldHelper 'GNSS_BUFFER_SIZE\s*==\s*index' 'numeric field extraction does not reject missing terminators'
Assert-Match $transactionHelper 'candidate\s*=\s*\*target\s*;' 'transaction parser does not begin from a private candidate'
Assert-Match $transactionHelper 'gps_gnrmc_parse\(\(char\s*\*\)buffer,\s*&candidate\)' 'transaction parser does not isolate RMC writes'
Assert-Match $transactionHelper 'gps_gngga_parse\(\(char\s*\*\)buffer,\s*&candidate\)' 'transaction parser does not isolate GGA writes'
Assert-Match $transactionHelper '\*target\s*=\s*candidate\s*;' 'transaction parser does not commit only after semantic success'
Assert-Match $dataParser 'gnss_parse_sentence_transaction\(gps_rmc_buffer,\s*gps_rmc_length,\s*gps_rmc_truncated,\s*&gnss\)' 'RMC production parsing does not use captured length/truncation transactionally'
Assert-Match $dataParser 'gnss_parse_sentence_transaction\(gps_gga_buffer,\s*gps_gga_length,\s*gps_gga_truncated,\s*&gnss\)' 'GGA production parsing does not use captured length/truncation transactionally'
Assert-Match $hostParser 'return\s+gnss_parse_sentence_transaction\(buffer,\s*length,\s*0U,\s*parsed\)\s*;' 'host parsing does not share the production transaction helper'
...
Assert-Match $dataParser 'gnss_parse_sentence_transaction\(gps_rmc_buffer[\s\S]+?else\s*\{\s*gnss\.rmc_sequence\s*=\s*gnss_next_sequence\(gnss\.rmc_sequence\)' 'format-valid RMC does not advance its parse sequence after commit'
Assert-Match $dataParser 'gnss_parse_sentence_transaction\(gps_gga_buffer[\s\S]+?else\s*\{\s*gnss\.gga_sequence\s*=\s*gnss_next_sequence\(gnss\.gga_sequence\)' 'format-valid GGA does not advance its parse sequence after commit'
