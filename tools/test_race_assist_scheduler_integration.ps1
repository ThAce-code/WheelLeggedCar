...
try {
    ...
    @(
        "app_config.h", "single_gap_config.h", "app_types.h", "app_scheduler.c", "app_scheduler.h",
...
#endif
'@ | Set-Content -LiteralPath (Join-Path $tempRoot "intercore_control.h") -NoNewline
...
#endif
'@ | Set-Content -LiteralPath (Join-Path $tempRoot "motion_command_router.h") -NoNewline
...
#endif
'@ | Set-Content -LiteralPath (Join-Path $tempRoot "single_gap_pose_source.h") -NoNewline
...
void intercore_control_update(uint32 now_ms) { (void)now_ms; }
void motion_command_router_update(uint32 now_ms, uint8 safety_fault)
{
    (void)now_ms; (void)safety_fault;
}
void single_gap_pose_source_update(uint32 now_ms) { (void)now_ms; }
